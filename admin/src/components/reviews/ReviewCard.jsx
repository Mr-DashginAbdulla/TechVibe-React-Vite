...
const ReviewCard = ({ review, productName, productImage, onDelete }) => {
  ...
  return (
    <div className="bg-card rounded-[14px] border border-border p-[14px] sm:p-[18px]">
      <div className="flex gap-[12px]">
        ...
        <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-[6px] shrink-0">
              <div className="flex gap-px">
                {[1, 2, 3, 4, 5].map((s) => (
                  <Star
                    key={s}
                    className={`w-[14px] h-[14px] ${s <= review.rating ? "text-[#F59E0B] fill-[#F59E0B]" : "text-border"}`}
                  />
                ))}
              </div>
              <button
          ...
          <p className="text-[13px] sm:text-[14px] text-foreground mt-[8px] line-clamp-2 sm:line-clamp-none">
            {review.comment}
          </p>
          <div className="flex items-center gap-[12px] mt-[8px] text-[11px] sm:text-[12px] text-muted-foreground">
